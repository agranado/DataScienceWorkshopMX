...
ggplot(data=titanic,aes(x=Sex,fill=Survived))+geom_bar(position="fill") + ylab("Frequency")
...
ggplot(data = titanic,aes(x=Pclass,fill=Survived))+geom_bar(position="fill")+ylab("Frequency")
...
age_name_sorted <- titanic %>% filter(Sex=="male") %>% arrange(desc(Age)) %>% select(Age,Name) 
```
...
titanic %>%  mutate(Age_Bracket = ifelse(Age < 18, 'Minor','Major')) %>%  group_by(Survived,Age_Bracket) %>%  summarise(pnt = n())
