...
```{r echo=F, message = F, warning = F, fig5, fig.height = 11, fig.width = 10 }
...
plot_deaths %>% ggplot(aes(x =Diff,y = Cases,color=Country.Region)) + geom_path() + geom_point() + theme_wsj(base_size = 16)  +  guides(color = FALSE)   + geom_text_repel(data =plot_deaths %>% filter(Date == last(Date)), aes(label = label_countries_deaths), nudge_x =2,xlim = c(max(plot_deaths$Diff[plot_deaths$Country.Region=='Iran']) + 7,NA), hjust = 0, direction ='y',segment.size = 0.2, segment.colour = 'gray50',size = 4)  + xlim(0,max(plot_deaths$Diff)+4)
